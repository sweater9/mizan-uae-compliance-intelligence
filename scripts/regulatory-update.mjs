...
try {
  ...
  if (sources.length === 0) {
    result.failures.push({ sourceId: "configuration", error: "No enabled regulatory sources are registered." });
    await finish("failed");
    console.error("Mizan regulatory update failed: no enabled regulatory sources are registered.");
    process.exitCode = 1;
  } else {
